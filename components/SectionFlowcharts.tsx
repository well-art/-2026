...
import { Activity, Share2, GitMerge, MousePointerClick, ChevronRight } from 'lucide-react';
import { DocItem } from '../types';
import { FLOWCHART_CATEGORIES } from '../data';
import { checkIsNew } from '../utils';

interface SectionFlowchartsProps {
  data: DocItem[];
  onItemClick: (item: DocItem) => void;
  searchTerm: string;
}

const SectionFlowcharts: React.FC<SectionFlowchartsProps> = ({ data, onItemClick, searchTerm }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-visible min-h-[300px] relative z-20">
            ...
            {FLOWCHART_CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => handleCategoryClick(cat.id)} className="flex flex-col items-center justify-center p-4 md:p-6 border border-gray-100 rounded-xl hover:border-brand-green hover:shadow-lg hover:shadow-brand-green/10 hover:-translate-y-1 transition-all duration-300 group bg-white h-32 md:h-40">
                <div className="bg-gray-50 p-3 md:p-4 rounded-full mb-2 md:mb-3 group-hover:bg-brand-green transition-colors duration-300">
                  <Share2 size={24} className="text-gray-400 group-hover:text-white transition-colors duration-300 md:w-8 md:h-8" />
                </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0 pr-2">
                        <div className="flex items-start gap-2 mb-1">
                            <GitMerge size={16} className="text-brand-green/70 flex-shrink-0 mt-1" />
