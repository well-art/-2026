...
import { Table, Layers, Archive, MousePointerClick, ChevronRight } from 'lucide-react';
import { DocItem } from '../types';
import { FORM_CATEGORIES } from '../data';
...
const SectionForms: React.FC<SectionFormsProps> = ({ data, onItemClick, searchTerm }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-visible min-h-[300px] relative z-10">
      <div className="sticky top-16 z-30 p-4 md:p-5 border-b border-gray-100 flex flex-col justify-center bg-white rounded-t-xl shadow-sm">
          ...
          {!isSearching && (
            <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 mt-2 touch-pan-x w-[calc(100%+2rem)] -mx-4 px-4 md:w-full md:mx-0 md:px-0">
              <button onClick={() => handleCategoryClick(null)} className={`flex-shrink-0 whitespace-nowrap px-3 py-1 rounded-full text-sm font-medium border transition-colors ${selectedCategoryId === null ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-500 border-gray-200 hover:border-brand-green hover:text-brand-green'}`}>所有表格</button>
              {FORM_CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => handleCategoryClick(cat.id)} className={`flex-shrink-0 whitespace-nowrap px-3 py-1 rounded-full text-sm font-medium border transition-colors ${selectedCategoryId === cat.id ? 'bg-brand-green text-white border-brand-green' : 'bg-white text-gray-500 border-gray-200 hover:border-brand-green hover:text-brand-green'}`}>{cat.name}</button>
              ))}
            </div>
        )}
      </div>
      <div className="p-4 md:p-6">
        {!isSearching && !isCategorySelected && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {FORM_CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => handleCategoryClick(cat.id)} className="flex flex-col items-center justify-center p-4 md:p-6 border border-gray-100 rounded-xl hover:border-brand-green hover:shadow-lg hover:shadow-brand-green/10 hover:-translate-y-1 transition-all duration-300 group bg-white h-32 md:h-40">
                  <div className="bg-gray-50 p-3 md:p-4 rounded-full mb-2 md:mb-3 group-hover:bg-brand-green transition-colors duration-300">
                  <Layers size={24} className="text-gray-400 group-hover:text-white transition-colors duration-300 md:w-8 md:h-8" />
                </div>
            ...
            {displayData.length === 0 ? (
              ...
              <div className="grid gap-3">
                {displayData.map((item) => (
                  <div key={item.id} onClick={() => onItemClick(item)} className="p-4 border border-gray-100 rounded-lg hover:bg-green-50/30 hover:border-brand-green/30 transition-all group cursor-pointer bg-white shadow-sm hover:shadow-md">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0 pr-2">
                        <div className="flex items-start gap-2 mb-1">
                            <Archive size={16} className="text-brand-green/70 flex-shrink-0 mt-1" />
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0">
                          {checkIsNew(item.date) && <span className="bg-red-100 text-red-600 text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 mb-1">NEW</span>}
